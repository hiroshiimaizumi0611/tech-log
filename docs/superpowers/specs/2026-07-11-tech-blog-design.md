...
- 実名とオリジナルイラストを表示する。
- イラストは、ユーザーが後で提供する顔写真などの参考画像から作る。
- 実装中は中立的な仮画像を使用できるが、本番公開前にユーザー承認済みイラストへ差し替える。
...
値が未提供のSNSリンクは表示しない。メールアドレスと著者画像が未確定でもローカル実装と検証は進められるが、Production公開の完了条件には含める。
