...
5. Appを含むPluginでは、チャットの`@`メンションまたは追加メニューから内包Appを選択する
6. 最初は低リスクな読取依頼で確認する
...
公開直前にPublic Repositoryの未解決Issue数を確認する。各Issueの出力は番号・タイトル・URLだけにし、3件未満の場合は一覧とは別の最終行で取得できた件数が少ないことを伝える。本文と画面例は実在件数に合わせ、非公開または存在しないIssueを例として作らない。
...
2. Appを含むPluginで内包Appを選択する`@`メンションまたは追加メニュー
